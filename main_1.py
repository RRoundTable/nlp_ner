...
def iteration_model(model, dataset, parameter, train=True):
    ...
    for morph, ne_dict, character, seq_len, char_len, label, step in dataset.get_data_batch_size(
        ...
        if train and step % 100 == 0:
            ...
            if step % 100 == 0:
                print('[Test step: {:>4}] cost = {:>.9} Accuracy = {:>.6}'.format(step + 1, avg_cost / (step + 1),
                                                                                  100.0 * avg_correct / float(
                                                                                      total_labels)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iteration', type=int, default=5, help='fork 명령어를 사용할때 iteration 값에 매칭되는 모델이 로드됩니다.')
    ...
    parser.add_argument('--necessary_file', type=str, default="necessary.pkl")  # 존재하는 NER 품사 태그 사전
    parser.add_argument('--train_lines', type=int, default=50, required=False, help='Maximum train lines')

    parser.add_argument('--epochs', type=int, default=100, required=False, help='Epoch value')
    parser.add_argument('--batch_size', type=int, default=30, required=False, help='Batch size')
    parser.add_argument('--learning_rate', type=float, default=0.01, required=False, help='Set learning rate')
    parser.add_argument('--keep_prob', type=float, default=0.5, required=False, help='Dropout_rate')

    parser.add_argument("--word_embedding_size", type=int, default=30, required=False,
                        help='Word, WordPos Embedding Size')
    parser.add_argument("--char_embedding_size", type=int, default=20, required=False, help='Char Embedding Size')
    parser.add_argument("--tag_embedding_size", type=int, default=16, required=False, help='Tag Embedding Size')

    parser.add_argument('--lstm_units', type=int, default=24, required=False, help='Hidden unit size')
    parser.add_argument('--char_lstm_units', type=int, default=40, required=False, help='Hidden unit size for Char rnn')
