...
class Dataset:
    def __init__(self, parameter, extern_data):
        ...
        self.extern_data = extern_data # 빈 list

        if parameter["mode"] == "train" and not os.path.exists(parameter["necessary_file"]):
            self._make_necessary_data_by_train_data() # train mode이고 necessary_file이 존재하지 않는다면
        else:
            with open(parameter["necessary_file"], 'rb') as f:
                self.necessary_data = pickle.load(f) # 존재하는 NER 품사 태그 사전
    ...
    def _make_necessary_data_by_train_data(self):
        ...
        print("-------------_make_necessary_data_by_train_data(self):--------------")
        ...
        #print("necessary_data[word] :", necessary_data["word"])
        #print(" necessary_data[character] : ", necessary_data["character"])
        #print("necessary_data[ner_tag] : ",necessary_data["ner_tag"])
        #print("necessary_data[ner_morph_tag]  : ",necessary_data["ner_morph_tag"] )


        print("-------------_make_necessary_data_by_train_data(self): END--------------")
    def make_input_data(self, extern_data=None):
        print("_______________make_input_data()___________________")
        morphs = []
        ne_dicts = []
        characters = []
        labels = []
        ...
        # print("self.ne_dicts  : ",self.ne_dicts )
        # print("self.characters : ",self.characters )
        # print("self.sequence_lengths : ",self.sequence_lengths)
        # print(" self.character_lengths : ", self.character_lengths)
        # print(" self.labels : ", self.labels)

        print("_______________make_input_data() END___________________")

    def get_data_batch_size(self, n, train=True):
        if train:
            for i, step in enumerate(range(0, self.parameter["train_lines"], n)): # n은 batch_size
    ...
    def _read_data_file(self, pre=True, extern_data=None): # 빈 list =! None
        if extern_data is not None:
