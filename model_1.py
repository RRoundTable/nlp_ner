...
class Model:
    ...
    def build_model(self):
        ...
        self._embeddings.append(
            tf.nn.embedding_lookup(self._embedding_matrix[0], self.morph))  # self.morph : word index
        self._embeddings.append(
            tf.nn.embedding_lookup(self._embedding_matrix[1], self.character))  # self.character: character
        ...
        character_emb_rnn, _, _ = self._build_birnn_model(character_embedding, char_len,
                                                          self.parameter["char_lstm_units"], self.dropout_rate,
        ...
        all_data_emb = tf.concat([all_data_emb, character_emb_rnn],
                                 axis=2)  # shape (30,180,135) sentence_length(180) 55+80=135

        # 모든 데이터를 가져와서 Bi-RNN 실시
        sentence_output, W, B = self._build_birnn_model(all_data_emb, self.sequence, self.parameter["lstm_units"],
                                                        self.dropout_rate, scope="all_data_layer")
        # sentence_output shape =(?,48) // 48=2 * lstm_units
        # W shape =(48,30)
        # B shape =(30,)

        weights = tf.get_variable(name='weights', shape=(sentence_output.get_shape()[-1], 15),
    ...
    def _build_birnn_model(self, target, seq_len, lstm_units, keep_prob, last=False, scope="layer"):
        ...
        if last:
            ...
            with tf.variable_scope("forward_" + scope):
                ...
                # lstm_fw_cell = tf.contrib.rnn.OutputProjectionWrapper(lstm_fw_cell, output_size=lstm_units) # 24 -> 40
                lstm_fw_cell = self._build_stacked_cell(lstm_units, keep_prob, depth=5)
            ...
            with tf.variable_scope("backward_" + scope):
                ...
                # lstm_bw_cell = tf.contrib.rnn.OutputProjectionWrapper(lstm_bw_cell, output_size=lstm_units)
                lstm_bw_cell = self._build_stacked_cell(lstm_units, keep_prob, depth=5)
            with tf.variable_scope("birnn-lstm_" + scope):
                # return (outputs, output_states)
                # _output = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, dtype=tf.float32,
                #                                           inputs=target, sequence_length=seq_len, scope="rnn_" + scope)
                ...
                outputs = tf.reshape(_output, shape=[-1, 2 * lstm_units])  # crf 할 때 필요
                outputs = _output
        #     print("----------------------------------------------------------------------------")
    ...
    def _build_seq2seq_layer(self, target):
        with tf.variable_scope('seq2seq_loss'):
            masks = tf.sequence_mask(lengths=self.sequence, maxlen=self.parameter["sentence_length"],
                                     dtype=tf.float32)  # self._X_length : , max_len : self.parameter["sentence_length"]
    ...
    def _build_output_layer(self, cost):
        with tf.variable_scope("output_layer"):
            train_op = tf.train.AdamOptimizer(self.parameter["learning_rate"]).minimize(cost,
                                                                                        global_step=self.global_step)
        return train_op

    def sparse_cross_entropy_loss(self):
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits,
                                                              labels=self.targets)  # self.logits 어디서 온 것인지
